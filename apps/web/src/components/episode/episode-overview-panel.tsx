...
export function EpisodeOverviewPanel({ episode }: { episode: Episode }): React.JSX.Element {
  const recentCharacters = episode.characters;
  const recentWorlds = episode.worlds;
    ...
    <section className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          ...
          <ReferenceBlock
            title="Locations"
            items={episode.locations.map((location) => location.name)}
          />
          <ReferenceBlock title="Props" items={episode.props.map((prop) => prop.name)} />
          <ReferenceBlock title="Assets" items={episode.assets.map((asset) => asset.name)} />
        </CardContent>
      </Card>
    </section>
  );
